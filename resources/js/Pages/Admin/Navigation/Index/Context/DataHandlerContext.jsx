...
const DataHandlerContextProvider = ({ children }) => {
  ...
  /**
   * Función para aplanar la estructura jerárquica de navegación
   * Convierte el árbol de navegación en una lista plana
   */
  const flattenNavigationTree = useCallback((items) => {
    const flattened = [];
    
    const flatten = (nodes, level = 0) => {
      nodes.forEach(node => {
        // Agregar el nodo actual con información de nivel
        flattened.push({
          ...node,
          level, // Agregar nivel para futura referencia si es necesario
        });
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      const response = await axios.get(route('api.v1.admin.navigation.index'));

      if (response.status === 200) {
        // Aplanar la estructura jerárquica para mostrar todos los elementos
        const flattenedLinks = flattenNavigationTree(response.data.links);
        setData(flattenedLinks);
      } else {
        setData([]);
      }
    } catch (error) {
  ...
  }, [setData, setLoading, flattenNavigationTree]);
