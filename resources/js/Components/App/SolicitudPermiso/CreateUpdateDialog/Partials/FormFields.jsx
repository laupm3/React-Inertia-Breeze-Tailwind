...
import { usePermisoFormManager } from '@/Pages/Admin/SolicitudPermisos/Index/Hooks/usePermisoFormManager';

// Importar componentes del lado del usuario (temporalmente)
import {
...
/**
 * FormFields refactorizado para usar los hooks del admin
 * Siguiendo el patrón exitoso del lado del usuario
 */
const FormFields = forwardRef(({ form, updateForm, model, isLoading, error, onStateChange }, ref) => {
    const isEditing = Boolean(model);
    
    // Usar el hook principal del admin
    const hookResult = usePermisoFormManager(model);
    ...
    }, [model?.id, isInitializing, initializeForEdit]); // Añadir dependencias necesarias
    ...
    const handleEmpleadoChange = React.useCallback((empleadoId) => {
        ...
        if (isNaN(numericEmpleadoId) || numericEmpleadoId <= 0) {
            console.error('❌ FormFields - Invalid empleado ID:', empleadoId);
            return;
        }
    ...
    const existingFilesForDisplay = React.useMemo(() => {
        const files = isEditing && fullModel?.files ? fullModel.files : [];
        return files;
    ...
    return (
        <div className="space-y-4">
            ...
            <PermisoSelectionSection
                ...
                dateRange={dateRange}
                empleadoId={formData.empleado_id} // Pasar empleado seleccionado para estadísticas
            />
            ...
            {isSubmitting && (
                <div className="bg-blue-50 border border-blue-200 rounded p-3 text-center">
                    <div className="text-sm text-blue-700">
                        {isEditing ? 'Actualizando permiso...' : 'Creando permiso...'}
                    </div>
                </div>
            )}
        </div>
