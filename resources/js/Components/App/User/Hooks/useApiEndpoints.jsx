...
const useApiEndpoints = (model) => {
    return useMemo(() => ({
        ...
        show: model ? route("api.v1.admin.users.show", { user: model }) : null,
        store: route("api.v1.admin.users.store"),
        update: model ? route("api.v1.admin.users.update", { user: model }) : null,
        delete: model ? route("api.v1.admin.users.destroy", { user: model }) : null,
        statuses: route("api.v1.admin.users.available-statuses"),
    }), [model]);
}
