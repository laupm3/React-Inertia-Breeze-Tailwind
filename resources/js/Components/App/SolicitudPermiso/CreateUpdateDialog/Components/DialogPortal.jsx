...
import { useDataHandler } from '@/Pages/Admin/SolicitudPermisos/Index/Context/DataHandlerContext';
import FormFields from '@/Components/App/SolicitudPermiso/CreateUpdateDialog/Partials/FormFields';
...
export default function DialogPortal({ model, onOpenChange }) {
    ...
    // Actualizar estado cuando se monta el componente
    React.useEffect(() => {
    ...
    return (
        <DialogContent className="sm:max-w-[700px] max-h-[80vh] flex flex-col bg-white dark:bg-custom-blackLight">
            <DialogHeader className="flex-shrink-0">
                ...
                <DialogDescription className="text-custom-gray-dark dark:text-custom-gray-light">
                    {isEditing 
                        ? 'Modifica los detalles de la solicitud de permiso.' 
                        : 'Complete los detalles para crear una nueva solicitud de permiso.'
                    }
                </DialogDescription>
            </DialogHeader>
            ...
            <DialogFooter className="flex-shrink-0 flex flex-col sm:flex-row gap-2 pt-4 border-t border-custom-gray-light dark:border-custom-gray-darker">
                ...
                >
                    {/* <Icon 
                        name={isSubmitting ? "Loader2" : (isEditing ? "Save" : "")} 
