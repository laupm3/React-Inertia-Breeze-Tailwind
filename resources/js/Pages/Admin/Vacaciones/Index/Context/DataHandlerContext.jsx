...
const DataHandlerContextProvider = ({ children }) => {
  ...
  /**
   * Obtiene todos los datos de vacaciones desde la API automáticamente
   */
  const fetchData = useCallback(async () => {
    setLoading(true);
    try {
      const response = await axios.get(route('api.v1.admin.vacaciones.index'));
      
      if (response.status === 200) {
        const validVacaciones = (response.data.solicitudes || []).filter(
          solicitud => solicitud && solicitud.id !== undefined
        );
        setData(validVacaciones);
  ...
  const handleSubmit = useCallback(async (formDataToSubmit) => {
    ...
    try {
      const endpoint = currentFormModel ? 
        route('api.v1.admin.vacaciones.update', currentFormModel.id) :
        route('api.v1.admin.vacaciones.store');
      ...
      if (response.status === 200 || response.status === 201) {
        toast.success(currentFormModel ? 'Solicitud de vacaciones actualizada correctamente' : 'Solicitud de vacaciones creada correctamente');
  ...
  const handleDelete = useCallback(async (model) => {
    ...
    try {
      const response = await axios.delete(route('api.v1.admin.vacaciones.destroy', model.id));
      ...
      setFormError(true);
      toast.error('Error al eliminar la solicitud de vacaciones');
      return false;
    } finally {
      setIsFormProcessing(false);
    }
