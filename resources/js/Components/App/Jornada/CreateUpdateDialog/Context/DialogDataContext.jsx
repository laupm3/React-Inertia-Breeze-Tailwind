...
const DialogDataContextProvider = ({
  ...
  const [prepareFn, setPrepareFn] = useState(() => () => {}); // Función de preparación antes del envío
  ...
  /**
   * Registra una función de preparación que se ejecutará antes del envío
   * @param {Function} fn - Función que se ejecutará antes del envío
   */
  const registerPrepareFn = useCallback((fn) => {
    setPrepareFn(() => fn);
  }, []);
  ...
  const handleSubmit = useCallback(() => {
    // Ejecutar función de preparación si existe
    if (prepareFn) {
      prepareFn();
    }
    ...
    setTimeout(() => {
      ...
      handleRequest();
    }, 50); // Pequeño delay para asegurar sincronización
  }, [handleValidation, handleRequest, prepareFn, form.data]);
