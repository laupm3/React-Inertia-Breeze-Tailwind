...
class ContratoStoreRequest extends FormRequest
{
    ...
    public function rules(): array
    {
        return [
            ...
            'n_expediente' => ['required', 'string', 'unique:contratos,n_expediente'],
    ...
    {
        $input = $this->all();
        // Si fecha_fin es string vacío, convertir a null
        if (array_key_exists('fecha_fin', $input) && ($input['fecha_fin'] === '' || $input['fecha_fin'] === '--' || $input['fecha_fin'] === '-  -')) {
            $this->merge([
                'fecha_fin' => null,
            ]);
        }
