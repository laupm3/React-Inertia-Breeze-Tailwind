...
export default function EditFileForm({ onClose, currentFolder, fileContextMenu, isModalEditFileOpen, setIsModalEditFileOpen}) {

  const [uploading, setUploading] = useState(false);
  const [uploadResult, setUploadResult] = useState(null);
  const [selectSeguridadArchivo, setSelectSeguridadArchivo] = useState(fileContextMenu.nivel_seguridad_id ?? 1);
  const [selectNivelAcceso, setSelectNivelAcceso] = useState(fileContextMenu.nivel_acceso_id ?? 4);
  const [nombreArchivo, setNombreArchivo] = useState(fileContextMenu.nombre ?? "Nombre de archivo");
  const inputNombreArchivoRef = useRef(null);

  useEffect( () => {
   inputNombreArchivoRef.current.focus(); 
  }, []);
  ...
  //setIsModalEditFileOpen(true);
  setUploading(true);

  try {
    const response = await axios.patch(`/admin/files/update/${fileContextMenu.id}`, {
    ...
    toast.success("Archivo actualizado correctamente!");
        ...
        toast.error("Error: No se ha podido actualizar el archivo!");
        /* setUploadResult(
      ...
      <Card className="w-full max-h-[94dvh] shadow-lg rounded-lg overflow-hidden px-2 pt-4 pb-2">
        ...
        <div className="pl-4 mb-6">
          <h2 className="text-xl font-semibold text-gray-700 dark:text-white mb-2">
            Editar archivo <strong className="text-custom-orange">{fileContextMenu.nombre}</strong>
          </h2>
        </div>
            ...
            <div className="space-y-2">
                <div>
                    <Label htmlFor="security-type" className="text-sm font-medium text-gray-800 dark:text-gray-300 mr-4">
                        Nombre del archivo <span className="text-custom-orange text-lg">*</span>
                    </Label>
                    <input type="text" placeholder="Nueva carpeta" autoFocus
                        className="w-3/5 rounded-md border-gray-300 text-sm shadow-sm focus:ring-orange-500 focus:border-orange-500 dark:text-gray-900" 
                        value={nombreArchivo}
                        onChange={(e) => setNombreArchivo(e.target.value)}
                        ref={inputNombreArchivoRef}
                    />
                </div>
