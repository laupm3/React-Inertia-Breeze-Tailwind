...
class NotificarNominaEliminada implements ShouldQueue
{
     ...
     * Handle the event.
     */
    public function handle(NominaFileDeleted $event): void
    {
        $file = $event->file;
        ...
        // Obtenemos el usuario autenticado o el que eliminó el archivo
        $user = Auth::user() ?? $file->user;

        // Obtenemos los usuarios que deben recibir la notificación
        $recipients = User::role(['admin', 'rrhh'])->get();

        // Evitar duplicados si el usuario que eliminó está en la lista
        ...
        $notificationData = [
            ...
            'notification_id' => uniqid('nomina_deleted_'),
            'action_url' => route('admin.files.index'),
            'brevo_params' => [
        ...
        $message = "Se ha eliminado la nómina: {$file->nombre}";
        ...
        foreach ($recipients as $recipient) {
            try {
                ...
                Log::info('Notificación de nómina eliminada enviada', [
                    'recipient' => $recipient->id,
                    'file_id' => $file->id
                ]);
            } catch (\Exception $e) {
                Log::error('Error al enviar notificación de nómina eliminada', [
                    'error' => $e->getMessage(),
                    'recipient' => $recipient->id,
                    'file_id' => $file->id
                ]);
