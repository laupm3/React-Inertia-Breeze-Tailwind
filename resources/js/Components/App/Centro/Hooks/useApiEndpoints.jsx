...
const useApiEndpoints = (model) => {
    return useMemo(() => ({
        show: model ? route("api.v1.admin.centros.show", { centro: model }) : null,
        store: route("api.v1.admin.centros.store"),
        update: model ? route("api.v1.admin.centros.update", { centro: model }) : null,
        delete: model ? route("api.v1.admin.centros.destroy", { centro: model }) : null,
    }), [model]);
}
