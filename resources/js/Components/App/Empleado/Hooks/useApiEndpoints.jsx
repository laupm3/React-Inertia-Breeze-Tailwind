...
const useApiEndpoints = (model) => {
    return useMemo(() => ({
        show: model ? route("api.v1.admin.empleados.show", { empleado: model }) : null,
        store: route("api.v1.admin.empleados.store"),
        update: model ? route("api.v1.admin.empleados.update", { empleado: model }) : null,
        delete: model ? route("api.v1.admin.empleados.destroy", { empleado: model }) : null,
        contratosVigentes: model ? route("api.v1.admin.empleados.contratos", { empleado: model }) : null,
    }), [model]);
}
