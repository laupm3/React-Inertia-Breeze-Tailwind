...
import { usePermisoFormManager } from '@/Pages/User/SolicitudPermisos/Hooks/usePermisoFormManager';
import { useFileManager } from '@/Pages/User/SolicitudPermisos/Hooks/useFileManager';
import { useFormValidation } from '@/Pages/User/SolicitudPermisos/Hooks/useFormValidation';
import { usePermisoApi } from '@/Pages/User/SolicitudPermisos/Hooks/usePermisoApi';


/**
 * CreateUpdateDialog Component - Versión Refactorizada
 * 
 * Modal dialog para crear y editar solicitudes de permiso
 ...
 * @param {Object|null} props.permisoToEdit - Datos del permiso para editar (null para crear)
...
export default function CreateUpdateDialog({
    ...
    /**
     * Cargar datos completos del permiso para edición
     */
    const loadPermisoCompleto = async (permisoId) => {
        try {
            setIsLoadingPermiso(true);
            const response = await axios.get(`/api/v1/user/solicitudes/${permisoId}`);
            const permiso = response.data.solicitudPermiso;
            setPermisoCompleto(permiso);
            return permiso;
        } catch (error) {
            console.error('Error al cargar permiso completo:', error);
            return null;
        } finally {
        ...
        <Dialog open={open} onOpenChange={onOpenChange}>
            <DialogContent className='bg-custom-white dark:bg-custom-blackLight max-w-3xl w-full max-h-[90vh] flex flex-col'>
                <DialogHeader className="flex-shrink-0">
                    <DialogTitle>
                        {isEditing ? 'Editar Permiso' : 'Solicitar Permiso'}
                    </DialogTitle>
                    <DialogDescription>
                        {isEditing 
                            ? 'Modifica los datos de tu solicitud de permiso existente.' 
                            : 'Completa el formulario para solicitar un nuevo permiso.'
                        }
                    </DialogDescription>
                </DialogHeader>
                ...
                <div className="flex-1 overflow-y-auto">
                    ...
                    {isEditing && isLoadingPermiso ? (
                        ...
                        <form className="space-y-3 pr-2">
                            {/* Sección de selección de permiso */}
                            <PermisoSelectionSection
                ...
                <DialogFooter className="px-6 pb-4 pt-6 flex justify-end gap-4 flex-shrink-0 border-t border-custom-gray-light dark:border-custom-gray-darker">
                    ...
                    >
                        {isSubmitting 
                            ...
                            : (isEditing ? 'Guardar Cambios' : 'Solicitar Permiso')
                        }
                    </Button>
                </DialogFooter>
            </DialogContent>
