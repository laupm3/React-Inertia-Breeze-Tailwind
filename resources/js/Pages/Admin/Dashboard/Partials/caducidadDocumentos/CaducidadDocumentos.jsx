...
function CaducidadDocumentos({ data, date }) {

  const empleadoCard = (row) => {
    // Verificamos si la fecha de caducidad y los días restantes son válidos.
    const hasValidDate = row.fecha_caducidad && row.dias_restantes !== null;
        ...
        <div className='flex flex-row w-full items-center gap-2'>
          ...
          {hasValidDate && (
            <span className='flex flex-row gap-1 text-xs text-custom-orange ml-auto bg-custom-orange/20 rounded-full px-2 py-1'>
              ...
              {format(new Date(row.fecha_caducidad), "dd MMM yyyy", { locale: es })}
            </span>
          )}
        </div>
          ...
          {hasValidDate ? (
            <>
              <span className='text-sm'>
                El <span className='font-bold'>{row.document_type}</span> expira el {format(new Date(row.fecha_caducidad), "d 'de' MMMM 'de' yyyy", { locale: es })}. Quedan {Math.floor(row.dias_restantes)} días.
              </span>
            </>
          ) : (
            <span className='text-sm text-muted-foreground'>
              Este documento no tiene fecha de caducidad.
            </span>
          )}
        </div>
      </span>
  ...
  return (
    <>
      <section className='flex w-full'>
        <BlockCard title='Caducidad de Documentos' className="h-full">
          {data && data.expiring_in_days_counts ? (
...
export default CaducidadDocumentos