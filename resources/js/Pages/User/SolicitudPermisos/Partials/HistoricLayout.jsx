...
import VistaPermiso from '@/Pages/User/SolicitudPermisos/Components/VistaPermiso';
import CreateUpdateDialog from '@/Pages/User/SolicitudPermisos/Partials/CreateUpdateDialog';
...
function HistoricLayout({ solicitudesPermiso: initialSolicitudes = [] }) {
  ...
  // Cargar solicitudes de permiso desde la API
  const fetchSolicitudes = async () => {
    try {
      setLoading(true);
      setError(null);
      const response = await axios.get('/api/v1/user/solicitudes');
      
      // El controlador devuelve { solicitudesPermiso: [...] }
      const data = response.data.solicitudesPermiso || [];
  ...
  const confirmDeletePermiso = async () => {
    ...
    try {
      if (isDeleteAction) {
        ...
        await axios.delete(`/api/v1/user/solicitudes/${permisoToDelete.id}`);
        
        // Mostrar notificación de éxito
        toast.success('Solicitud eliminada correctamente', {
          description: `La solicitud de permiso "${permisoToDelete.tipo_permiso_nombre || permisoToDelete.permiso?.nombre}" ha sido eliminada.`
        });
      } else {
        // Hacer la petición POST para solicitar cancelación
        ...
        toast.success('Solicitud de cancelación enviada', {
          description: `Se ha enviado la solicitud de cancelación para "${permisoToDelete.tipo_permiso_nombre || permisoToDelete.permiso?.nombre}".`
        });
      }
    ...
    <div className='flex flex-col xl:flex-row justify-center w-full p-14 gap-20'>
      ...
      <DecisionModal
        ...
        content={
          permisoToDelete ? 
            permisoToDelete.estado?.nombre === "Solicitado" ?
              `Esta acción eliminará permanentemente la solicitud de permiso "${permisoToDelete.tipo_permiso_nombre || permisoToDelete.permiso?.nombre}". Esta acción no se puede deshacer.` :
              `Esta acción solicitará la cancelación de la solicitud de permiso "${permisoToDelete.tipo_permiso_nombre || permisoToDelete.permiso?.nombre}". El administrador revisará tu solicitud de cancelación.`
            :
            permisoToDelete?.estado?.nombre === "Solicitado" ?
              'Esta acción eliminará permanentemente la solicitud de permiso. Esta acción no se puede deshacer.' :
              'Esta acción solicitará la cancelación de la solicitud de permiso. El administrador revisará tu solicitud de cancelación.'
        }
        icon={
          permisoToDelete?.estado?.nombre === "Solicitado" 
