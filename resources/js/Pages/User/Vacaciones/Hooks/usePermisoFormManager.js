...
import { defaultPermisoValues, TIPO_PERMISO_VACACIONES_ID } from '@/Pages/User/Vacaciones/Schema/PermisoSchema';
...
export function usePermisoFormManager(permisoToEdit = null) {
    ...
    // Efecto para inicializar automáticamente el tipo de permiso de vacaciones
    useEffect(() => {
        if (!isEditing && !isInitializing) {
            // Para nuevas solicitudes, asegurar que siempre tenga el ID de vacaciones
            setFormData(prev => ({
                ...prev,
                permiso_id: TIPO_PERMISO_VACACIONES_ID
            }));
            
            // También crear un objeto selectedPermiso mock para mantener consistencia
            setSelectedPermiso({
                id: TIPO_PERMISO_VACACIONES_ID,
                nombre: 'Vacaciones',
