...
import Dialog from "@/Pages/Admin/Departamentos/Partials/CreateUpdateDialog";
 ...
 * @returns {JSX.Element}
 */
export default function TableToolbar({ table, entity = 'departamentos' }) {
  const [filter, setFilter] = useState("");
  const [perPageDropdownOpen, setPerPageDropdownOpen] = useState(false); // Estado para controlar la apertura del menú de elementos por página
  ...
  const [selectedAdjuntos, setSelectedAdjuntos] = useState([]); // Estado para los empleados seleccionados
  const [selectedDepartamentos, setSelectedDepartamentos] = useState([]); // Estado para los empleados seleccionados
  const [selectedCentros, setSelectedCentros] = useState([]); // Estado para los empleados seleccionados
  const [selectedManagers, setSelectedManagers] = useState([]); // Estado para los empleados seleccionados
  ...
  const renderMainButtons = () => (
    <div className="flex gap-4">
      ...
      >
        <span className="hidden sm:inline">{t('tables.añadirdepartamento')}</span>
      ...
      >
        ...
        <div className="flex flex-col gap-2 p-2 w-full min-w-[20rem] max-w-[20rem]">
          ...
          <div className="flex flex-col gap-y-3">
            <RenderDinamicFilter
              key={`departamento-filter`}
              table={table}
              columnKey="nombre"
              placeholder={t('tables.departamento')}
              getLabel={(nombre) => nombre}
              selectedValues={selectedDepartamentos}
              setSelectedValues={setSelectedDepartamentos}
            />
            <RenderDinamicFilter
              key={`centros-filter`}
              table={table}
            ...
            <RenderDinamicFilter
              ...
              table={table}
              columnKey="manager"
              placeholder={t('tables.manager')}
              getLabel={(manager) => `${manager.nombre} ${manager.primer_apellido}`}
              selectedValues={selectedManagers}
              setSelectedValues={setSelectedManagers}
            />
            <RenderDinamicFilter
              key={`adjunto-filter`}
              table={table}
              ...
              getLabel={(adjunto) => `${adjunto.nombre} ${adjunto.primer_apellido}`}
